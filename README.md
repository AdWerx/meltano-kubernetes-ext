...
kubernetes is A meltano utility extension for kubernetes that wraps the `` command.
...
```shell
poetry run kubernetes_extension --help
poetry run kubernetes_extension describe --format=yaml
poetry run kubernetes_invoker --help # if you have are wrapping another tool
```
